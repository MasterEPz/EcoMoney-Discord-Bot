...
# ---------------- F1 CAP LIMIT ----------------
CAP_LIMIT = 215_000_000  # change if you want
...
# ---------------- VIEW ACCOUNTS ----------------
@bot.tree.command(name="viewaccs")
async def viewaccs(interaction: discord.Interaction):
    async with aiosqlite.connect(DB) as db:
        cursor = await db.execute(
            "SELECT account_name, external FROM accounts WHERE user_id=?",
            (interaction.user.id,)
        )
        data = await cursor.fetchall()
...
async def deposit(interaction: discord.Interaction, account: str, amount: int):
    async with aiosqlite.connect(DB) as db:
        ...
        if is_f1 and budget + amount >= CAP_LIMIT:
            await interaction.response.send_message("❌ F1 cap reached (withdraw-only mode).")
            return

        if external < amount:
            await interaction.response.send_message("❌ Not enough external money.")
            return
    ...
    await interaction.response.send_message(f"✅ Deposited ${amount} into {account}")
...
async def withdraw(interaction: discord.Interaction, account: str, amount: int):
    ...
    await interaction.response.send_message(f"💸 Withdrawn ${amount} from {account}")
...
async def transfer(interaction: discord.Interaction, account: str, user: discord.User, amount: int):
    async with aiosqlite.connect(DB) as db:
        ...
        # receiver (first account of target user)
        cursor = await db.execute(
        ...
        sender_external = sender[0]
        receiver_name, receiver_external = receiver

        if sender_external < amount:
        ...
        await db.execute(
            ...
            (sender_external - amount, interaction.user.id, account)
        )

        # update receiver
        await db.execute(
            "UPDATE accounts SET external=? WHERE user_id=? AND account_name=?",
            (receiver_external + amount, user.id, receiver_name)
        )

        await db.commit()

    await interaction.response.send_message(
        f"🔁 {account} sent ${amount} to {user.name}"
    )
UPGRADE_DATA = {
...
async def upgrade(interaction: discord.Interaction, level: int):
    async with aiosqlite.connect(DB) as db:
        ...
        # ---------------- GET F1 ACCOUNT ----------------
        cursor = await db.execute(
            "SELECT account_name, budget, level FROM accounts WHERE user_id=? AND is_f1=1",
            (interaction.user.id,)
        )
        data = await cursor.fetchone()
        ...
        name, budget, current_level = data

        # safety fallback (if DB has no level column yet)
        current_level = current_level or 0

        # ---------------- VALIDATE LEVEL ----------------
        if level not in UPGRADE_DATA:
            await interaction.response.send_message("❌ Invalid level. Use 1, 2, or 3.")
            return
        ...
        # ---------------- MONEY CHECK ----------------
        if budget < cost:
            await interaction.response.send_message("❌ Not enough money in F1 account.")
            return
    ...
    await interaction.response.send_message(
        f"🏎 F1 Upgrade Level {level} applied (+{value}) | Cost: ${cost}"
    )
