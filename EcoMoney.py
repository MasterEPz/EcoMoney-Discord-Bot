...
async def on_ready():
    async with aiosqlite.connect(DB) as db:
        ...
        await db.execute("""
    INSERT INTO transactions (user_id, action, account, amount, target_user)
    VALUES (?, 'TRANSFER', ?, ?, ?)
""", (interaction.user.id, account, amount, user.id))
...
async def leaderboard(interaction: discord.Interaction, top: int = 10, mode: str = "all"):
    ...
    async with aiosqlite.connect(DB) as db:
        ...
        if mode == "all":
            cursor = await db.execute("""
                SELECT user_id, SUM(budget) as total
                FROM accounts
                GROUP BY user_id
                ORDER BY total DESC
                LIMIT ?
            ...
            title = "🏆 Player Leaderboard (All Accounts)"
        ...
        # ---------------- F1 MODE ----------------
            ...
            title = "🏎 F1 Player Leaderboard"

        else:
            await interaction.response.send_message("❌ Mode must be `all` or `f1`.")
            return
