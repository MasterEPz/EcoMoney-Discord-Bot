...
async def upgrade(interaction: discord.Interaction, level: int):
    ...
    async def viewaccs(interaction:discord.Interaction, user: discord.User, account: str):
        
    async with aiosqlite.connect(DB) as db:
    cursor = await db.
    execute("SELECT account_name, budget, external, is_f1, level FROM accounts WHERE user_id=? AND account_name=?",
            (user.id, account)
        )
    data = await cursor.fetchone()
    ...
    name, budget, external, is_f1, level = data

    msg = f"💰 **Account Info for {user.name}**\n\n"
    msg += f"📁 Account: **{name}**\n"
    msg += f"💵 Budget: ${budget}\n"
    msg += f"💳 External: ${external}\n"
    msg += f"🏎 F1: {'Yes' if is_f1 else 'No'}\n"
    msg += f"📊 Level: {level}"
